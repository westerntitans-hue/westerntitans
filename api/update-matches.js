export default async function handler(req, res) {
  ...
  try {
    ...
    if (!GITHUB_TOKEN) {
      console.log('Matches update (no GitHub token):', matches);
      return res.status(200).json({ success: true, message: 'Matches updated (logged)' });
    }
    
    const data = { matches };
    const content = Buffer.from(JSON.stringify(data, null, 2)).toString('base64');
    
    const getResponse = await fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/public/matchInfo.json`, {
      headers: {
    ...
    const updateResponse = await fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/public/matchInfo.json`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${GITHUB_TOKEN}`,
        'Accept': 'application/vnd.github.v3+json',
